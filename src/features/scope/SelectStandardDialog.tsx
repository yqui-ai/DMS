...
export function SelectStandardDialog({
  ...
  const inScopeIds = useMemo(() => new Set(waveObjects.filter((w) => w.inScope).map((w) => w.migrationObjectId)), [waveObjects]);
  ...
  return (
    ...
    >
      <p className="text-sm text-muted mb-3">The standard Master Data starter set — check the objects to add to this wave's scope.</p>
